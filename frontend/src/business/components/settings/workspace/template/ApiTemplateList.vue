  ...
  <el-card class="table-card">
    ...
    <template v-slot:header>
      <ms-table-header :condition.sync="condition" @search="initTableData" @create="handleCreate"
                       :create-tip="$t('custom_field.template_create')" :title="$t('custom_field.api_template')"/>
    </template>
    ...
    <api-template-edit ref="templateEdit" @refresh="initTableData"/>
...
export default {
  ...
  methods: {
    ...
    handleDelete(data) {
      this.result = this.$get('field/template/api/delete/' + data.id,  () => {
